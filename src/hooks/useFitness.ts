...
const DAILY_EXERCISES: Record<string, Array<{ exercise: string; duration: number; sets?: number; reps?: number; notes?: string }>> = {
  '塑形': [
...
export function useFitness() {
  ...
  const saveFitnessData = useCallback(async (data: {
    ...
    if (effectiveOnline) {
      try {
        ...
        await (supabase.from('fitness_data') as any).upsert({ id, user_id: userId, current_weight: data.currentWeight, target_weight: data.targetWeight, height: data.height, focus_area: data.focusArea, body_parts: data.bodyParts }, { onConflict: 'user_id' });
  ...
  const generatePlan = useCallback(async (fd: FitnessData): Promise<FitnessPlan> => {
    ...
    const exercises = DAILY_EXERCISES[focus] ?? DAILY_EXERCISES['综合']!;
    const daily = Array.from({ length: 30 }, (_, i) => ({ day: i + 1, ...exercises[i % 7]! }));
    const monthly = [
    ...
    const plan: FitnessPlan = { id, userId, fitnessDataId: fd.id, name: `${focus}定制方案`, planData: { daily, monthly }, isAccepted: false, acceptedAt: null, createdAt: new Date().toISOString() };
    const db = getDatabase();
  ...
  const logExercise = useCallback(async (data: {
    ...
    const log: ExerciseLog = { id, userId, date: today, exerciseType: data.exerciseType, durationMinutes: data.durationMinutes ?? null, intensity: data.intensity ?? null, caloriesBurned: data.caloriesBurned ?? null, notes: data.notes ?? null, createdAt: new Date().toISOString() };
    const db = getDatabase();
    ...
    if (effectiveOnline) {
      try {
        const supabase = getSupabaseBrowserClient();
        await (supabase.from('exercise_logs') as any).insert({ id, user_id: userId, date: today, exercise_type: data.exerciseType, duration_minutes: data.durationMinutes, intensity: data.intensity, calories_burned: data.caloriesBurned, notes: data.notes });
