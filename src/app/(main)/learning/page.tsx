...
export default function LearningPage(){
  ...
  return(<div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 animate-fade-in"><div className="mb-6"><h1 className="text-2xl font-bold flex items-center gap-2 text-purple-500"><GraduationCap className="h-7 w-7"/>{t('learning.title')}</h1><p className="text-sm mt-1 text-slate-400">勾选子板块 · 学习计划 · 思维导图 · 英文阅读(50篇长文)</p></div>
    <div className="module-card mb-6" style={{'--module-accent':'#8b5cf6'} as React.CSSProperties}><h2 className="section-title" style={{'--module-accent':'#8b5cf6'} as React.CSSProperties}><BookOpen className="h-5 w-5 text-purple-500"/>{t('learning.categories')}</h2><div className="flex flex-wrap gap-2">{LEARNING_CATEGORIES.map(cat=>{const cd=categories.find(c=>c.category===cat);const isA=cd?.isActive??false;const tc=todayLogs.some(l=>l.category===cat);return(<button key={cat} onClick={()=>toggleCategory(cat)} className={cn('flex items-center gap-2 px-4 py-2.5 rounded-lg transition-all border-2 text-sm font-medium',isA?'shadow-sm':'')} style={{background:isA?'#8b5cf612':'var(--color-surface-alt)',borderColor:isA?'#8b5cf6':'var(--color-border)',color:isA?'#7c3aed':'var(--color-text-secondary)'}}><span>{cat}</span>{tc&&<CheckCircle2 className="h-3.5 w-3.5 text-emerald-500"/>}</button>);})}</div></div>
    ...
    <div className="flex items-center gap-1 mb-4 p-1 rounded-lg bg-slate-100">{['plan','mindmap',...(isIelts(cat)?['reading']:[])].map(tab=><button key={tab} onClick={()=>setAt(p=>({...p,[cat]:tab}))} className={cn('flex-1 py-1.5 rounded-md text-xs font-medium transition-all',ct===tab?'bg-white text-purple-600 shadow-sm':'text-slate-500 hover:text-slate-700')}>{tab==='plan'?'📋 学习计划':tab==='mindmap'?'🧠 思维导图':'📖 英文阅读'}</button>)}</div>
